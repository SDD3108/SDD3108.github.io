...
class Reg{
    ...
    checkingLogin(login,email){
        // console.log('Сохраняю пользователей в localStorage:', this.arr)
        // console.log(this.arr.some(user => user.log == login || user.mail == email));
        if(this.arr.some(user => user.log == login || user.mail == email) == true && this.arr.length == 0){
            return false
        }
...
btn.addEventListener('click',()=>{
    ...
    if(!one || !two || !lol || !lel || !four){
        ...
        console.log('norm');
        console.log(reg.checkingLogin(one,two));
